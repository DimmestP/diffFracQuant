...
```{r ROC-curve}
ground_truth = c(rep("equal", 150),
                 rep(c("sup", "pel"), each = 75))

latent_model_ROC_data <- calc_ROC_values(latent_model_results$p.value,
                                        latent_model_results$ratio,
                                        ground_truth,
                                        latent = TRUE) %>%
  mutate(model = "latent")

DESeq_model_ROC_data <- calc_ROC_values(DESeq_model_results$padj,
                                        DESeq_model_results$log2FoldChange,
                                        ground_truth) %>%
  mutate(model = "DESeq")

ggplot(DESeq_model_ROC_data %>%
         bind_rows(latent_model_ROC_data)) +
  geom_line(aes(x = false_positive, y = true_positive, colour = model)) +
...
detect_diff_exp_latent <- latent_model_results %>%
  select(gene_name, ratio, p.value) %>%
