...
Bayesian_model_simulated_random_results <- read_tsv(here("./data/output/diff_frac_models/bayesian_simulated_result.tsv"))
```
...
encode_data_sets_subsampled <- encode_data_sets %>%
  filter(gene_id %in% chosen_encode_genes)

write_tsv(encode_data_sets_subsampled, here("data/output/subsampled_experimental_counts/ENCODE_dataset_subsampled.tsv"))
...
Bayesian_vs_ground_truth_data_set <- simulate_fractions_log_fold_ground_truth %>%
  inner_join(Bayesian_model_simulated_random_results %>%
               transmute(gene_name, 
                         condition,
