...
sup_pel_simulated_fractions_skewed <- tibble(sup_frac = c(rep(0.75,num_gene - 20),
                                                              rep(0.25,20)),
                                                 pel_frac = 1 - sup_frac)

total_sup_pel_simulated_mean_counts_skew <- tibble(gene_name = 1:length(sup_pel_simulated_fractions_skewed$sup_frac),
                                              total = rlnorm(length(gene_name),
                                                  meanlog = total_count_model$estimate[1], 
                                                  sdlog = total_count_model$estimate[2]),
...
total_sup_pel_simulated_noisy_counts_skew <- tibble(gene_name = rep(total_sup_pel_simulated_mean_counts_skew$gene_name, num_rep),
                                               rep = rep(1:num_rep, each = num_gene),
            total = sample_noisy_counts(total_sup_pel_simulated_mean_counts_skew$total,
                                  phi,
                                  num_rep,
                                  tot_sup_pel_scale_factor$total),
            sup = sample_noisy_counts(total_sup_pel_simulated_mean_counts_skew$sup,
                                  phi,
                                  num_rep,
                                  tot_sup_pel_scale_factor$sup),
            pel = sample_noisy_counts(total_sup_pel_simulated_mean_counts_skew$pel,
                                  phi,
                                  num_rep,
                                  tot_sup_pel_scale_factor$pel)) %>%
  mutate(total = as.integer(total),
