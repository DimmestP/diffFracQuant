...
model{
  ...
  for(con in 1:NCON){
    ...
    pel_latent[con] ~ lognormal(pel_gamma_alpha, pel_gamma_beta);
      ...
      for(rep in 1:NREP){
          // fractions
          sup_obs[, rep, con] ~ neg_binomial_2(sup_scale_factor[rep, con] *
                                                  sup_latent[con],
                                                phi);
                                              
        pel_obs[, rep, con] ~ neg_binomial_2(pel_scale_factor[rep, con] *
                                                  pel_latent[con],
                                                phi);
  
        // count distn negative binomial with specified means
        // total
        tot_obs[, rep, con] ~ neg_binomial_2(sup_latent[con] + pel_latent[con], phi);
      }
  }
}',
...
extract_summary_statistics <- function(stan_model){
    ...
    list(
        ...
        scale_factors = stan_model %>% 
          spread_draws(pel_scale_factor[rep, condition],
