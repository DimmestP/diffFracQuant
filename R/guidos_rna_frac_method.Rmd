...
model{
  ...
  total_scale_factor ~ normal(1.5,0.5);
  pel_scale_factor ~ normal(1.5,0.5);
  sup_scale_factor ~ normal(1.5,0.5);
...
test_diff_frac <- function(stan_model){
  stan_model %>% 
    ...
    summarise(p.value = sum( pel_latent / sup_latent > 1) / length(sup_latent))
}
