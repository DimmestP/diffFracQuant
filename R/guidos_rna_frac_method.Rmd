...
parameters {
  // Normalising factors
  real<lower=0> tot_norm_factor[NREP];
  real<lower=0, upper=1> sup_norm_factor[NREP];
...
model{
  // normalising factors
  tot_norm_factor ~ exponential(1);
  sup_norm_factor ~ beta(1,1);
  ...
  for(rna in 1:NRNA){
    for(rep in 1:NREP){ 
      // fractions
      sup_obs[rna,rep] ~ poisson(sup_norm_factor[rep] * sup_latent[rna]);
      pel_obs[rna,rep] ~ poisson((1 -sup_norm_factor[rep]) * pel_latent[rna]);
  
      // count distn negative binomial with specified means
      // total
      tot_obs[rna,rep] ~ neg_binomial_2(tot_norm_factor[rep] * (sup_latent[rna] + pel_latent[rna]), tot_phi);
    }
  }
}
