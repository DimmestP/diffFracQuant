...
parameters {
  // Normalising factors
  real<lower=0> scale_factor_mean;
  real<lower=0> tot_scale_factor[NCON, NREP];
  real<lower=0> sup_scale_factor[NCON, NREP];
...
for(con in 1:NCON){
    ...
    scale_factor_mean ~ normal(10, 0.5);
    tot_scale_factor[con] ~ normal(scale_factor_mean, 0.1);
    ...
    for(rep in 1:NREP){
          // fractions
          sup_obs[con, rep] ~ neg_binomial_2(sup_scale_factor[con, rep] * sup_latent[con],
                                                phi[con, 2]);
                                              
        pel_obs[con, rep] ~ neg_binomial_2(pel_scale_factor[con, rep] * pel_latent[con],
                                                phi[con, 3]);
  
        // count distn negative binomial with specified means
        // total
        tot_obs[con, rep] ~ neg_binomial_2(tot_scale_factor[con, rep] * (sup_latent[con] + pel_latent[con]),
					 	phi[con, 1]);
      }
  }
}
generated quantities{
	...
	for(con in 1:NCON){
		sup_obs_sim[con] = neg_binomial_2_rng(sup_scale_factor[con, 2] * sup_latent[con],
                                                phi[con, 2]);

        	pel_obs_sim[con] = neg_binomial_2_rng(pel_scale_factor[con, 3] * pel_latent[con],
                                                phi[con, 3]);

		tot_obs_sim[con] = neg_binomial_2_rng(tot_scale_factor[con, 1] * (sup_latent[con] + pel_latent[con]),
						phi[con, 1]);
	}
}',
