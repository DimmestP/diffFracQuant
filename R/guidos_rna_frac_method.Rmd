...
model{
  for(rna in 1:NRNA){
    ...
    for(rep in 1:NREP){
      // fractions
      sup_obs[rna,rep] ~ neg_binomial_2( sup_latent[rna], tot_phi);
      pel_obs[rna,rep] ~ neg_binomial_2( pel_latent[rna], tot_phi);
  
      // count distn negative binomial with specified means
      // total
      tot_obs[rna,rep] ~ neg_binomial_2(sup_latent[rna] + pel_latent[rna], tot_phi);
    }
  }
}
...
stan_count_model <- total_sup_pel_simulated_noisy_counts %>%
  filter(total > 5,
         sup > 5,
         pel > 5) %>%
    fit_mixingTSP(, stan_mixing=stan_count_model,iter=500)
