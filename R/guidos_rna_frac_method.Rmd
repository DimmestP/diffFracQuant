...
parameters {
  ...
  real<lower=0> sup_norm_factor[NREP];
  real<lower=0> pel_norm_factor[NREP];
...
model{
  ...
  for(rna in 1:NRNA){
    for(rep in 1:NREP){ 
      ...
      pel_obs[rna,rep] ~ poisson(pel_norm_factor[rep] * pel_latent[rna]);
...
make_mixingTSP(total_sup_pel_simulated_noisy_counts)
